...
def getTracks():
    ...
    # genres = sp.current_user_saved_tracks(limit=50, offset=0)['items'][count]["track"]["album"]["genres"]

    # album_name = sp.current_user_saved_tracks(limit=50, offset=0)['items'][count]['track']['album']['name']
    while count < 10:
    ...
    saved_dict = {
        "date_added" : timestamp_saved,
        "album_name" : album_names,
        "artist" : artist_names,
        "songs" : tracks
    }
    
    saved_df = pd.DataFrame(saved_dict, columns = ["date_added", "album_name", "artist", "songs"])
